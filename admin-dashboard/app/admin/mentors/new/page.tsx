...
export default function MentorFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const endpoint = isEditing ? `/api/mentors/${mentorId}` : '/api/mentors';
      ...
      if (selectedSpecialties.length > 0) {
        await fetch(`/api/mentors/${mentor.id}/specialties`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={isEditing ? 'Edit Mentor' : 'Add New Mentor'}
        description={
          isEditing ? 'Update mentor information and specialties' : 'Create a new mentor profile'
        }
      />
