...
// Hook to track which group chats the user has joined (local + remote)
export function useJoinedGroupChats(userId: string | undefined) {
  ...
  const markJoined = (chatId: string) => {
    setJoinedIds((prev) => new Set([...prev, chatId]));
  };

  const markLeft = (chatId: string) => {
    setJoinedIds((prev) => {
  ...
  return { joinedIds, loading, markJoined, markLeft };
}