...
from .fallback_utils import (
    ...
    safe_deduplicate_by_field,  # FIXED: Import central implementation
)

logger = get_logger(__name__)


# FIXED: Removed duplicate safe_deduplicate_careers - use central implementation from fallback_utils
# This prevents divergence between two implementations and follows DRY principle
...
class LLMService:
    ...
    Design Notes:
        - use_mock is a CONFIGURATION FLAG set during __init__
        - NOT mutated at runtime to avoid shared state corruption
        - Each API error triggers fallback for that specific call only
        - Transient errors (rate limit, timeout, quota) don't change mode
        - After service restart with corrected config, LLM mode resumes
        - This prevents "permanent silent failures" from quota resets or key updates
    ...
    Status:
        ...
        - ✓ Safe fallback on any failure (NO STATE MUTATION)
        ...
        if not self._use_mock:
            try:
                ...
                self.client = openai.OpenAI(
                    ...
                    timeout=config.TIMEOUT_SECONDS,  # FIXED: Pass timeout for cross-platform support
                )
                self.logger.info(
        ...
        try:
            ...
            return {
                ...
                "recommended_careers": safe_deduplicate_by_field(
                    parsed.get("recommended_careers", []),
                    field_key="role",
                    keep_first=True,
                ),
    ...
    def analyze_skill_gaps(
        ...
        """
        Analyze skill gaps using LLM with optional RAG context for prerequisites and difficulty.
        ...
        Args:
            ...
            rag_context (Optional[str]): Knowledge base context with prerequisites and difficulty info
        ...
        try:
            ...
            prompt = self._build_gap_analysis_prompt(
                current_skills, target_role, required_skills, rag_context
            )
...
{{
    ...
    def generate_learning_roadmap(
        ...
        try:
            ...
            prompt = self._build_roadmap_prompt(
                target_role, missing_skills, current_experience, rag_context
            )
    ...
    def _safe_api_call(self, api_call_func):
        """
        Execute API call with comprehensive error handling, timeout protection, and retry logic.
        
        WARNING: This method does NOT mutate instance state (self.use_mock).
        Only the configuration flag set during __init__ determines mock behavior.
        Each transient error (quota, rate limit, timeout, network) triggers a
        one-time fallback for that specific call. Permanent errors (invalid key,
        model not found) are logged but still return None for fallback, allowing
        the system to recover if conditions change (e.g., service restart, key update).
        
        FIXED: Now includes:
        - Timeout protection using config.TIMEOUT_SECONDS (signal.SIGALRM on Unix)
        - Retry logic for transient failures (RATE_LIMIT, TIMEOUT) with 1s backoff
        
        Retries:
        - RATE_LIMIT: 1 retry after 1s delay
        - TIMEOUT: 1 retry after 1s delay 
        - Other transient errors: No retry, immediate fallback
        ...
        def timeout_handler(signum, frame):
            raise TimeoutError(f"API call exceeded {config.TIMEOUT_SECONDS}s timeout")
        
        last_error = None  # Track last error for final handling
        
        # Attempt API call with optional retry for transient errors
        for attempt in range(2):  # 2 attempts: initial + 1 retry
            try:
                # Setup timeout handler - prevents indefinite blocking if service hangs
                if hasattr(signal, 'SIGALRM'):
                    ...
                    # Fallback for Windows: OpenAI client respects timeout via requests library
                    return api_call_func()
            
            except Exception as e:
                last_error = e
                llm_error = categorize_llm_error(e)
                is_transient = llm_error.error_type in (LLMErrorType.RATE_LIMIT, LLMErrorType.TIMEOUT)
                is_last_attempt = (attempt == 1)  # Second attempt (index 1) is last
                
                # Retry transient errors once with backoff
                if is_transient and not is_last_attempt:
                    self.logger.warning(
                        f"[RETRY] {llm_error.error_type.value.upper()} on attempt {attempt + 1}. "
                        f"Retrying after 1s backoff... Error: {llm_error}"
                    )
                    time.sleep(1)  # 1s backoff before retry
                    continue  # Try again
                
                # For non-transient or final attempt, handle error below and break
                break
        
        # Handle final error (after retries exhausted or non-transient)
        if last_error:
            llm_error = categorize_llm_error(last_error)
            
            # Log with categorized error type - NO STATE MUTATIONS
            if llm_error.error_type == LLMErrorType.QUOTA_EXCEEDED:
                self.logger.error(
                    f"[FALLBACK_MOCK] API QUOTA EXCEEDED (429). "
                    f"Account has insufficient credits/quota. Falling back to mock for this call. "
                    f"Check account quota and retry. Error: {llm_error}"
                )
                # NOTE: NOT setting self.use_mock = True - this is a transient error!
                # After quota reset, the next call should work again.
            
            elif llm_error.error_type == LLMErrorType.RATE_LIMIT:
                self.logger.warning(
                    f"[FALLBACK_MOCK] API RATE LIMITED. "
                    f"Too many requests. Falling back to mock for this call. "
                    f"Requests may succeed after backoff. Error: {llm_error}"
                )
                # Transient error - don't mutate state
            
            elif llm_error.error_type == LLMErrorType.API_KEY_INVALID:
                self.logger.error(
                    f"[FALLBACK_MOCK] Invalid API key (401). "
                    f"Check OPENROUTER_API_KEY configuration. "
                    f"Falling back to mock for this call. "
                    f"Will resume using LLM if key is updated and process restarted. "
                    f"Error: {llm_error}"
                )
                # Not mutating state - allows recovery on service restart with corrected key
            
            elif llm_error.error_type == LLMErrorType.MODEL_NOT_FOUND:
                self.logger.error(
                    f"[FALLBACK_MOCK] Model '{config.LLM_MODEL}' not found (404). "
                    f"Check LLM_MODEL configuration. "
                    f"Falling back to mock for this call. "
                    f"Error: {llm_error}"
                )
                # Not mutating state - allows recovery with config change and service restart
            
            elif llm_error.error_type == LLMErrorType.CONTEXT_LENGTH:
                self.logger.warning(
                    f"[FALLBACK_MOCK] Request exceeds context length. "
                    f"Prompt is too large for this model. "
                    f"Falling back to mock for this call. Error: {llm_error}"
                )
                # Transient - specific to this request
            
            elif llm_error.error_type == LLMErrorType.TIMEOUT:
                self.logger.warning(
                    f"[FALLBACK_MOCK] API call timed out. Connection took too long. "
                    f"Falling back to mock for this call. "
                    f"Next request may succeed. Error: {llm_error}"
                )
                # Transient network condition - don't mutate state
            ...
            return None  # Signal caller to use fallback (transient, not permanent)
    ...
    def _create_parse_error_fallback(
        ...
        FIXED: Uses explicit named parameters instead of fragile *args unpacking.
        This prevents silent failures from refactored call sites and IndexError on arg mismatch.
        
        Args:
            method_type: "recommendations", "gap_analysis", or "roadmap"
            user_profile, user_skills, count: For recommendations fallback
            current_skills, target_role, required_skills: For gap_analysis fallback
            target_role, missing_skills, current_experience: For roadmap fallback
        
        Returns:
            Valid fallback response matching expected schema
        
        Raises:
            ValueError: If required parameters for the method_type are missing
        """
        if method_type == "recommendations":
            if user_profile is None or user_skills is None or count is None:
                raise ValueError(
                    "For 'recommendations' fallback, must provide: user_profile, user_skills, count"
                )
            fallback = {
                "success": True,
                "recommended_careers": [
            ...
            if current_skills is None or target_role is None or required_skills is None:
                raise ValueError(
                    "For 'gap_analysis' fallback, must provide: current_skills, target_role, required_skills"
                )
            ...
            if target_role is None or missing_skills is None or current_experience is None:
                raise ValueError(
                    "For 'roadmap' fallback, must provide: target_role, missing_skills, current_experience"
                )
            ...
            raise ValueError(
                f"Unknown method type: {method_type}. Must be 'recommendations', 'gap_analysis', or 'roadmap'"
            )
...
1. Match user skills against required skills for each career
2. Consider difficulty progression from {user_profile.get('experience_level', 'mid')} level
3. Check prerequisites for recommended skills
4. Return careers that align with user's current level and growth potential
5. Prioritize careers with high market demand
"""
...
ANALYSIS FRAMEWORK:
1. Skill Match: Calculate overlap between user and required skills
2. Growth Potential: Can user reach this role with realistic learning?
3. Market Value: Is there strong demand for this role?
4. Difficulty Fit: Does difficulty align with user's level?

TASK:
1. Recommend {count} career paths that align with background
2. For each, identify specific required skills for success
3. Provide realistic confidence scores based on:
   - Current skill overlap (primary)
   - Learning curve fit (secondary)
   - Market demand (tertiary)
4. Consider career progression paths
...
{{
    "recommended_careers": [
        {{
            ...
            "market_demand": "high",
            "reasoning": "Why suited: user has X%, needs to learn Y%",
            "skills_overlap": ["Existing Skill1", "Existing Skill2"],
            "skills_to_learn": ["New Skill1", "New Skill2", "New Skill3"],
            "required_skills": ["Skill1", "Skill2", "Skill3"]
        }}
    ],
    "confidence_score": 0.85,
    "reasoning": "Overall assessment based on skill match and market alignment"
}}
        """
    ...
    def _build_gap_analysis_prompt(
        ...
        """Build detailed prompt for skill gap analysis with prerequisite awareness from RAG metadata."""
        rag_section = ""
        if rag_context:
            rag_section = f"""
KNOWLEDGE BASE WITH SKILL PREREQUISITES & DIFFICULTY:
{rag_context}

METADATA INSTRUCTIONS:
1. Extract difficulty level for each skill
2. Identify prerequisites from knowledge base
3. Suggest learning order based on dependencies
4. Recommend resources from knowledge base for each skill
5. Prioritize foundational skills before advanced ones
"""
...
2. For each gap, determine difficulty and prerequisites
3. Categorize gaps by priority (must-have vs nice-to-have)
4. Estimate learning timeline based on difficulty
5. Provide actionable recommendations with specific resources

ANALYSIS CRITERIA:
- "High Priority": Critical for day-1 success OR foundational skill
...
{{
    ...
    "learning_sequence": ["Start with this", "Then learn this"],
    "timeline_months": 6,
    "recommendations": [
