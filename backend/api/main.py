...
async def generate_roadmap(request: RoadmapGeneratorRequest):
    ...
    try:
        ...
        # Pull roadmap from agent output, not top-level schema field
        roadmap_agent_output = result.agent_outputs.get("roadmap")
        raw_phases = []
        if roadmap_agent_output and roadmap_agent_output.data:
            raw_phases = roadmap_agent_output.data.get("phases", [])
        
        logger.debug(f"RAW ROADMAP RESPONSE: {len(raw_phases)} phases from agent")
        if raw_phases:
            for i, phase in enumerate(raw_phases[:3]):
                phase_title = phase.get("title", f"Phase {i+1}") if isinstance(phase, dict) else phase
                logger.debug(f"  Phase {i}: {phase_title}")
        else:
        ...
        if not result or not raw_phases:
        ...
        for i, phase in enumerate(raw_phases):
            if isinstance(phase, dict):
                roadmap_steps.append({
                    ...
                    "duration_months": phase.get("duration_months", 2),
