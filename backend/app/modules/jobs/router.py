...
from scrapers import JobDetails as ScraperJobDetails
...
async def get_jobs(
    ...
    country_indeed: Optional[str] = Query(
        None, description="Optional override for JobSpy country_indeed (e.g., USA, FRANCE)"
    ),
    ...
    try:
        ...
        invalid_sites = [site for site in sites if site not in ALLOWED_SITES]
        if invalid_sites:
            raise HTTPException(
                status_code=400,
        ...
        if not search or not search.strip():
            sites = [site for site in sites if site not in ("naukri", "bayt")]
        ...
        # Build kwargs for scrape_jobs, only adding country_indeed if Indeed or Glassdoor is in sites
        scrape_kwargs = {
            "site_name": sites,
        ...
        if "indeed" in sites or "glassdoor" in sites:
            ...
            selected_country = (
                (country_indeed.strip().upper() if country_indeed and country_indeed.strip() else None)
                or inferred_country
                or "USA"
            )
        ...
        if job_type and job_type in JOB_TYPE_MAP:
            jobs_list = [job for job in jobs_list if job.get("job_type") == JOB_TYPE_MAP[job_type]]

        # Filter by is_remote if provided
        if is_remote is not None:
            jobs_list = [job for job in jobs_list if job.get("is_remote") == is_remote]
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Error scraping jobs: {exc}")
...
async def get_job_details(
    ...
    use_llm: bool = Query(
        False,
        description="If true, use LLM to extract skills/salary from available data",
    ),
):
    """
    ...
    if use_llm:
        try:
            ...
            # Try to get job page content through site-specific scraper first
            ...
            print(f"WARNING: LLM extraction failed: {exc}, falling back to regular scraping")
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Error scraping job details: {exc}")


@router.post("/api/extract-job-info")
async def extract_job_info(request: Request):
    ...
    try:
        ...
        if not should_fallback:
            ...
            salary = None
            if has_salary:
                # Use IndeedScraper's extract_salary method
